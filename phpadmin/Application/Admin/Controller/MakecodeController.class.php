...
////////////////////////////////////////////////////////////////////
	...
	class MakecodeController extends AdminBaseController
	{
		 ...
		 * 识别为多图片字段
		 */
		protected $imageFields = ['imageavatar', 'imagess'];
...
";
		...
		protected function getFieldType(& $v)
		{
			...
			// 指定后缀结尾且类型为varchar || char,多文件上传
			if ($this->isMatchSuffix($fieldsName, $this->imageFields) &&
				(($v['data_type'] == 'varchar') || $v['data_type'] == 'char')) {
				$inputType = "imagess";
			}
		...
		private function getJs($type, $col)
		{
			...
			switch ($type) {
				...
				case 'editor':
                    ...
                    KindEditor.ready(function ({$col['column_name']}) {
	                    {$col['column_name']}.create(\"textarea[name={$col['column_name']}]\", {
				            langType: 'zh-CN',
