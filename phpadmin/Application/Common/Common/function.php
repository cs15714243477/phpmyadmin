...
//检验网址
	...
	{
		$pattern_1 = "/(http|ftp|https):\/\/([\w\d\-_]+[\.\w\d\-_]+)[:\d+]?([\/]?[\w\/\.\?=&;%@#\+,]+)/i";
		$pattern_2 = "/([\w\d\-_]+[\.\w\d\-_]+)[:\d+]?([\/]?[\w\/\.\?=&;%@#\+,]+)/i";
...
//拼接 数组字符串 键 值 拼接
	...
	/*
	 * 创建redis  需要配置 DB_REDIS_IP  DB_REDIS_PORT
	 */
	function credis()
	{
		$redis = new \Redis();
		$redis->connect(C('DB_REDIS_IP'), C('DB_REDIS_PORT'), '60');
		return $redis;
	}
	...
	function geturl($a)
	{
		return __ROOT__ . 'index.php/home/' . $a;
	}
